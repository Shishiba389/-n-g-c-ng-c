...
import { ArrowLeft, ArrowUpRight, ChevronRight, LocateFixed, MapPin, Sparkles, Volume2, VolumeX } from 'lucide-react';
...
const chests:Chest[] = [
  ...
  {id:3,name:'Hòm Bánh Mì',kicker:'BÁNH MÌ • BÁNH KẸP',description:'50 biến thể bánh mì, từ cổ điển đến hiện đại.',accent:'#e7aa47',ids:[2,34,84,106,107,108,109,110,111,122,124]},
...
function Art({food}:{food:Food}) {
  ...
  if(generatedArt){
    return <div className="food-art food-placeholder" aria-hidden="true"/>;
  }
...
export default function Home() {
  ...
  const skip=()=>{cancelAnimationFrame(frame.current);if(winner)audio.current?.play((['item_reveal3_rare','item_reveal4_mythical','item_reveal5_legendary','item_reveal6_ancient','item_reveal6_ancient'] as const)[winner.rarity]);setScreen('result')};
  const map=winner?`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${winner.name} near ${location??district}`)}`:'#';
  const SoundButton=()=> <button className="icon-button" type="button" aria-label={sound?'Mute sound':'Enable sound'} aria-pressed={sound} onClick={()=>setSound(!sound)}>{sound?<Volume2/>:<VolumeX/>}</button>;
  const District=()=> <select aria-label="Area" value={district} onChange={event=>setDistrict(event.target.value)}>{districts.map(name=><option key={name}>{name}</option>)}</select>;

  if(screen==='home') return <main className="case-app home" id="main-content"><header className="case-header"><a className="wordmark" href={basePath||'/'}>ĂN GÌ CŨNG ĐƯỢC</a><span>OPEN • DISCOVER • FIND A PLACE</span><SoundButton/></header><section className="popular-head"><div><small>FIND YOUR NEXT BITE</small><h1>Pick a food chest</h1><p>A small spin to help you decide what to eat next.</p></div><button type="button">Browse all <ChevronRight aria-hidden="true" size={16}/></button></section><section className="chest-grid" aria-label="Food chests">{chests.map(chest=><button className="chest-card" key={chest.id} onClick={()=>{setSelected(chest);setScreen('detail')}} style={{'--accent':chest.accent} as React.CSSProperties}><b>{chest.kicker}</b><i>50 PICKS</i><img src={`${basePath}/chest-${chest.id}.png`} alt={`Biểu tượng ${chest.name}`}/><strong>{chest.name}</strong><small>{chest.description}</small></button>)}</section></main>;
  if(screen==='detail') return <main className="case-app detail" id="main-content" style={{'--accent':selected.accent} as React.CSSProperties}><header className="case-header"><button className="back-button" type="button" onClick={()=>setScreen('home')}><ArrowLeft aria-hidden="true" size={19}/>Back</button><span>Food chests / {selected.name}</span><SoundButton/></header><section className="case-hero"><div><small>{selected.kicker}</small><h1>{selected.name}</h1><p>{selected.description}</p><button className="open-case" type="button" onClick={open}><Sparkles aria-hidden="true" size={19}/>Open chest</button></div><div className="rates"><b>DROP RATES</b>{[['Blue · Common','64%',0],['Purple · Rare','22%',1],['Pink · Special','10%',2],['Gold · Secret','4%',4]].map(([name,rate,rarity])=><div key={name as string}><span style={{color:colors[rarity as number]}}>{name}</span><i><em style={{width:rate as string,background:colors[rarity as number]}}/></i><small>{rate}</small></div>)}</div><img src={`${basePath}/chest-${selected.id}.png`} alt={`Hình ${selected.name}`}/></section><section className="items"><div className="items-title"><div><small>IN THIS CHEST</small><h2>50 possible picks</h2></div><label>Area <District/></label></div><div className="item-grid">{items.map((food,index)=><article key={index} title={food.name} style={{'--rarity':colors[food.rarity]} as React.CSSProperties}>{food.rarity===4&&<span>★ SECRET</span>}<Art food={food}/><b>{food.name}</b></article>)}</div></section></main>;
  if(screen==='spin') return <main className="case-app spin" id="main-content"><div className="roulette" aria-label="Opening chest animation"><div className="spin-line"/><div className="spin-track" style={{transform:`translate3d(${spinOffset}px,0,0)`}}>{spinReel.map((food,index)=><div className="spin-item" key={index} style={{'--rarity':colors[food.rarity]} as React.CSSProperties}><Art food={food}/></div>)}</div></div><div className="spin-controls"><SoundButton/><button type="button" onClick={skip}>Skip</button></div></main>;
  return <main className="case-app result" id="main-content" style={{'--accent':selected.accent} as React.CSSProperties}>{winner&&<><small>YOUR PICK</small><h1>{winner.name}</h1><div className="result-food"><Art food={winner}/></div><div className="result-map"><div><MapPin aria-hidden="true" size={18}/><District/></div><button type="button" onClick={()=>navigator.geolocation?.getCurrentPosition(pos=>setLocation(`${pos.coords.latitude.toFixed(4)},${pos.coords.longitude.toFixed(4)}`),()=>setLocation(null))}><LocateFixed aria-hidden="true" size={16}/>Use current location</button></div><div className="result-actions"><a href={map} target="_blank" rel="noreferrer">FIND PLACES <ArrowUpRight aria-hidden="true" size={16}/></a><button type="button" onClick={()=>setScreen('detail')}>Keep exploring</button></div></>}</main>;
}
